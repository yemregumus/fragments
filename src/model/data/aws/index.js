...
async function deleteFragment(ownerId, id) {
  ...
  try {
    // Delete the object from Amazon S3 bucket
    await s3Client.send(s3Command);
    logger.info('Fragment data has been successfully deleted.');
  } catch (err) {
    const { Bucket, Key } = s3Params;
    logger.error(
      { err, Bucket, Key },
      'There was an error when trying to delete data from Amazon S3'
    );
  ...
  // Create DELETE command to send to DynamoDB
  const command = new DeleteCommand(dynamoDbParams);

  try {
    await ddbDocClient.send(command);
  } catch (err) {
    logger.warn({ err, dynamoDbParams }, 'error deleting fragment metadata from DynamoDB');
